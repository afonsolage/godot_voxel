...
void VoxelMap::set_block_channel_buffer(Vector3i bpos, const Ref<VoxelBuffer> buffer, unsigned int channel, unsigned int buffer_padding) {
	...
	VoxelBlock *block = get_block(bpos);
	Vector3i copy_size = buffer->get_size() - 2 * buffer_padding;
	if (block == NULL) {
		//TODO: Create a pool for voxel buffers?
		Ref<VoxelBuffer> voxels;
		voxels.instance();
		voxels->create(copy_size.x, copy_size.y, copy_size.z);
	...
	if (buffer_padding == 0) {
		...
		block->voxels->copy_from(**buffer, Vector3i(buffer_padding), Vector3i(buffer_padding) + copy_size, Vector3i(), channel);
	}
}
