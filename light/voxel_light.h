...
typedef HashMap<Vector3i, Vector<VoxelLightData>, Vector3iHasher> VoxelLightMap ;

static inline unsigned int get_art_light(unsigned int value) {
    return value & ARTIFICIAL_LIGHT_MASK;
}

static inline unsigned int get_nat_light(unsigned int value) {
    return (value & NATURAL_LIGHT_MASK) >> NATURAL_LIGHT_SHIFT;
}

static inline unsigned int set_art_light(unsigned int value, unsigned int new_value) {
    value &= ~ARTIFICIAL_LIGHT_MASK;
    value |= (new_value & ARTIFICIAL_LIGHT_MASK);
    return value;
}

static inline unsigned int set_nat_light(unsigned int value, unsigned int new_value) {
...
static inline int get_final_light(unsigned int value) {
    unsigned int art_light = get_art_light(value);
    unsigned int nat_light = get_nat_light(value);
